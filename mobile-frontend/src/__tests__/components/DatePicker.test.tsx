...
describe('DatePicker Component', () => {
  ...
  const tomorrowDate = new Date(new Date().setDate(new Date().getDate() + 1)).toLocaleDateString('en-GB', {
    day: 'numeric',
    month: 'short',
    year: 'numeric',
  });
  ...
  it('does not render the date picker modal on initial render', async () => {
    const { queryByTestId } = render(
     ...
     screen.debug();
     await waitFor(() => expect(queryByTestId('date_time_picker_modal')).not.toBeVisible());
  });
  ...
  it('renders the date picker modal after pressing the date', async () => {
    const { findByText, queryByTestId } = render(
     ...
     await waitFor(() => expect(queryByTestId('date_time_picker_modal')).toBeVisible());
  });
