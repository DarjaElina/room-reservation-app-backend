...
describe('QueryResult Component', () => {
  it('renders error message when error prop is true', async () => {
    const {findByText} = render(
      <QueryResult children={<></>} data={null} loading={false} error={{name: 'Error', message: 'An error occured.'} as ApolloError} />
    )
  ...
  it('renders loading indicator when loading prop is true', async () => {
    const {findByTestId} = render(
      <QueryResult children={<></>} data={null} loading={true} error={undefined} />
    )
  ...
  it('renders data when data is not null', async () => {
    const {findByText} = render(
      <QueryResult data={{breed: 'cat', name: 'Cherry Pie'}} loading={false} error={undefined} >
        <View><Text>Cat: Cherry Pie</Text></View>
      </QueryResult>
    )
  ...
  it('renders "nothing to show" message when data is null, error is undefined and loading is false', async () => {
    const {findByText} = render(
      <QueryResult children={<></>} data={null} loading={false} error={undefined} />
    )
