...
describe('BookingItem Component', () => {
  it('renders initial booking details correctly', async () => {
    ...
    <MockedProvider>
      <BookingItem startDate={new Date(new Date().setHours(9, 0, 0, 0))} endDate={new Date(new Date().setHours(10, 0, 0, 0))} roomCode="r-100" id="1" roomId="2"/>
    </MockedProvider>
    )

    expect(await findByText('r-100')).toBeDefined();
    expect(await findByText('Jan 19, 2025, 09:00 - Jan 19, 2025, 10:00')).toBeDefined();
    //screen.debug();
  });
