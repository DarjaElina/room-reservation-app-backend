...
describe('CheckBoxItem Component', () => {
  ...
  it('renders the item label', async () => {
    const {findByText} = render(
      <CheckBoxItem item={{value: 'ice cream', label: 'Ice cream'}} isActive={true} updatedCheckedValues={['ice cream', 'cheesecake', 'tiramisu']}
      setUpdatedCheckedValues={setUpdatedCheckedValues} onChange={onChange}
      />
    )
  ...
  it('activates the checkbox when pressed', async () => {
    ...
    await waitFor(() => expect(setUpdatedCheckedValues).toHaveBeenCalledWith(['ice cream']));
  });
  ...
  it('deactivates the checkbox when pressed', async () => {
    ...
    await waitFor(() => expect(setUpdatedCheckedValues).toHaveBeenCalledWith([]));
  });
});