...
import { UniqueConstraintError, ValidationError, ForeignKeyConstraintError, ConnectionError, AggregateError } from 'sequelize';

export const handleResolverErrors = (error: unknown) => {
  console.log(error);
  if (error instanceof UniqueConstraintError) {
    const violatedFields = error.errors.map(e => e.path);
    throw new GraphQLError(`The following fields must be unique: ${violatedFields.join(', ')}`, {
      extensions: {
        code: 'BAD_USER_INPUT',
        invalidArgs: violatedFields,
        details: error.errors.map(e => ({ message: e.message, path: e.path }))
      }
    });
  }
